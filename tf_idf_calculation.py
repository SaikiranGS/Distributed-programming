...
if rank == 0:   
    idf_result= open('path of your idf results file','rt').read()
    idf =  json.loads(idf_result)
    #print(type(idf))
    bcast_variable = idf
    #print(type(idf))
    dir_list = os.listdir('path of your tf results file')
    #print(dir_list)
...
for each_document in receive1:
    arr = os.listdir('path of your tf results file' + each_document)
    for each_file in arr:
        filename = 'path of your tf results file' + each_document +'\\'+each_file
